...
phpunit: ## Run all tests -> phpunit.xml
	$(DOCKER_COMPOSE) run -u ${PUID}:${PGID} --rm phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php /app/tests
.PHONY: phpunit

phpunit-reports: ## Run all tests with reports -> clover and coverage
	$(DOCKER_COMPOSE) run -u ${PUID}:${PGID} --rm -e XDEBUG_MODE=coverage phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php /app/tests phpunit --coverage-clover tests/reports/clover.xml --coverage-xml tests/reports/coverage-xml
.PHONY: phpunit-reports

phpunit-coverage: ## Run all tests to find test leaks
	$(DOCKER_COMPOSE) run -u ${PUID}:${PGID} --rm -e XDEBUG_MODE=coverage phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php /app/tests phpunit --coverage-text
.PHONY: phpunit-coverage

phpunit-coverage-without-integration: ## Runs just unit tests
	$(DOCKER_COMPOSE) run -u ${PUID}:${PGID} --rm -e XDEBUG_MODE=coverage phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php /app/tests/unit phpunit --coverage-text
.PHONY: phpunit-coverage-without-integration

phpunit-coverage-html: ## Run all tests to find test leaks html
	$(DOCKER_COMPOSE) run -u ${PUID}:${PGID} --rm -e XDEBUG_MODE=coverage phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php /app/tests phpunit --coverage-html tests/reports/coverage-html
